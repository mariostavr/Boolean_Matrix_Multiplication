                ...
                for (int k=blocking*i; k<(blocking*i)+blocking; k++){
                    for (int h=blocking*j; h<(blocking*j)+blocking; h++){
                        if (multiply(k, h, csr_row, csr_col, csc_row, csc_col)){
                            row_private.push_back(k);
                            col_private.push_back(h); 
                        }
                    }
                }
              c_coo_row = row_private;
              c_coo_col = col_private;
              row_private.clear();
              col_private.clear();
            }
...
int main(int argc, char** argv){
    ...
    //cout << "\nConverting FILTER to B - Sparse - CSR...";
    data_BS f_bcsr = blockSparse(a_rows, a_cols, blocking, f_csr_row, f_csr_col);
    int *fb_csr_row = new int[f_bcsr.block_rows + 1];
    int *fb_csr_col = new int[f_bcsr.block_nnz];
    convertBS(a_rows, a_cols, f_bcsr, fb_csr_row, fb_csr_col, f_csr_row, f_csr_col);
    //cout << "  Succesfully convertion FILTER to B - Sparse - CSR!\n";
    ...
    //============================================================ FilterR B Multiplication
    //cout << "\n\nMultiplying FILTER BCSR - BCSC...\n\n";
    ...
    int elements = f_bcsr.block_rows / numProcs;

    for (int i=(rank*elements); i<(rank*elements+elements); i++)
        for (int j=fb_csr_row[i]; j<fb_csr_row[i+1]; j++)
            if ((b_multiply(blocking,i, fb_csr_col[j], b_csr_row, b_csr_col, b_csc_row, b_csc_col, csr_row, csr_col, csc_row, csc_col))){
                bc_coo_row.push_back(i);
                bc_coo_col.push_back(f_csr_col[j]);
            }
